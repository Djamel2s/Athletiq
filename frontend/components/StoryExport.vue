<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
    <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl max-w-md w-full max-h-[95vh] overflow-y-auto p-5">
      <!-- Header -->
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-bold text-zinc-900 dark:text-white">Partager sur Instagram</h2>
        <button @click="emit('close')" class="p-1.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 transition">
      ...
      <div class="flex gap-3 mb-4">
        ...
        >
          ...
          <p class="text-xs text-center mt-1.5 font-medium text-zinc-600 dark:text-zinc-400">{{ t.label }}</p>
        </button>
      </div>

      <!-- Phone frame preview -->
      <div class="relative mx-auto w-full max-w-[260px] mb-5">
        <div class="rounded-[28px] border-[3px] border-zinc-300 dark:border-zinc-600 bg-black p-1.5 shadow-xl">
...
const templates = [
  { id: 'minimal' as const, label: 'Minimal', preview: 'linear-gradient(135deg, #f5f0eb 0%, #e8e0d6 100%)' },
  { id: 'bold' as const, label: 'Bold', preview: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)' },
  { id: 'dark' as const, label: 'Dark', preview: 'linear-gradient(135deg, #000000 0%, #1a1a1a 100%)' },
]
...
function drawMinimal(ctx: CanvasRenderingContext2D) {
  ...
  if (props.data.prsBeaten > 0) {
    ...
    ctx.fillStyle = accentDark
    ctx.textAlign = 'center'
    const prText = props.data.prsBeaten === 1
      ? '\uD83C\uDFC6  1 record battu'
  ...
  if (props.data.streak > 0) {
    ...
    ctx.fillStyle = textMuted
    ctx.textAlign = 'center'
    const streakText = props.data.streak === 1
      ? '1 semaine de suite'
      : `${props.data.streak} semaines de suite`
