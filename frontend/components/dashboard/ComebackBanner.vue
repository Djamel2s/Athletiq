<template>
  <Transition name="fade-slide">
    <div
      v-if="show"
      class="card-glass relative overflow-hidden"
    >
      <!-- Gradient border accent -->
      <div class="absolute inset-0 rounded-2xl bg-gradient-to-br from-sand-500/20 to-sand-600/20 pointer-events-none"></div>
      ...
      <div class="relative flex flex-col sm:flex-row items-start gap-4">
        <!-- Icon -->
        <div class="w-12 h-12 bg-gradient-to-br from-sand-500 to-sand-600 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm">
        ...
        <div class="flex-1 min-w-0 pr-6">
          ...
          <p class="text-xs text-primary-400 dark:text-primary-500 mt-1">
            Dernier entraînement il y a {{ daysSinceLastWorkout }} jour{{ daysSinceLastWorkout > 1 ? 's' : '' }}
          </p>
...
const modeLabel = computed(() => {
  switch (comebackMode.value) {
    case 'gentle': return 'Mode reprise douce'
    case 'progressive': return 'Mode reprise progressive'
    case 'rebuild': return 'Mode reconstruction'
    default: return ''
  }
})
