<template>
  <div class="card-glass cursor-pointer hover:shadow-2xl transition-all" @click="$emit('click')">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h3 class="text-2xl font-semibold text-primary-900 dark:text-primary-100 mb-1">
          Suivi d'activité
        </h3>
        <p class="text-sm text-primary-600 dark:text-primary-400">
          {{ workoutsThisWeek }} entraînement{{ workoutsThisWeek > 1 ? 's' : '' }} cette semaine
        </p>
      </div>
    ...
    <div class="space-y-3">
      ...
      <div class="grid grid-cols-7 gap-2">
        <div
          ...
          :class="
            day.hasWorkout ? 'bg-gradient-primary shadow-sm' : 'bg-primary-100 dark:bg-primary-800'
          "
        >
          <!-- Tooltip au hover -->
          <div
            class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-primary-900 dark:bg-primary-100 text-white dark:text-primary-900 text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10"
          >
            {{ day.label }}
    ...
    <!-- Stats rapides -->
    <div
      class="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-primary-200 dark:border-primary-700"
    >
