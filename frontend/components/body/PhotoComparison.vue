  ...
  <div class="card-glass">
    ...
    <div v-if="beforePhoto && afterPhoto" class="flex justify-center gap-2 mb-4">
      <button
        ...
        :class="viewMode === 'slider' ? 'bg-gradient-primary text-white shadow-sm' : 'bg-primary-100 dark:bg-primary-800 text-primary-600 dark:text-primary-400'"
      >
        Slider
      </button>
      <button
        ...
        class="px-3 py-1.5 rounded-lg text-sm font-medium transition-all"
        :class="viewMode === 'split' ? 'bg-gradient-primary text-white shadow-sm' : 'bg-primary-100 dark:bg-primary-800 text-primary-600 dark:text-primary-400'"
      >
        Assemblé
      </button>
    </div>
    ...
    >
      <img :src="afterPhoto.photoUrl" :alt="'Après'" class="absolute inset-0 w-full h-full object-cover" draggable="false" />
      <div class="absolute inset-0" :style="{ clipPath: `inset(0 ${100 - sliderPosition}% 0 0)` }">
        <img :src="beforePhoto.photoUrl" :alt="'Avant'" class="absolute inset-0 w-full h-full object-cover" draggable="false" />
      </div>
      <div class="absolute top-0 bottom-0 w-0.5 bg-white shadow-lg pointer-events-none" :style="{ left: `${sliderPosition}%` }"></div>
      <div class="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-10 h-10 bg-white rounded-full shadow-xl flex items-center justify-center pointer-events-none" :style="{ left: `${sliderPosition}%` }">
    ...
    <div v-if="beforePhoto && afterPhoto && viewMode === 'split'" class="relative aspect-[3/4] rounded-2xl overflow-hidden bg-primary-100 dark:bg-primary-800">
      <div class="flex h-full">
        <!-- Left half (before) - right half of image -->
        <div class="w-1/2 h-full overflow-hidden relative">
          <img :src="beforePhoto.photoUrl" alt="Avant" class="absolute inset-0 w-[200%] h-full object-cover" style="object-position: center;" />
          <div class="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/60 to-transparent">
            ...
            <p class="text-white/70 text-[10px]">{{ formatDateShort(beforePhoto.workout?.date || beforePhoto.createdAt) }}</p>
          </div>
        </div>
        <!-- Divider -->
        ...
        <div class="w-1/2 h-full overflow-hidden relative">
          <img :src="afterPhoto.photoUrl" alt="Après" class="absolute inset-0 w-[200%] h-full object-cover" style="object-position: center; left: -100%;" />
          <div class="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/60 to-transparent text-right">
            <p class="text-white text-xs font-semibold uppercase tracking-wider">Après</p>
            <p class="text-white/70 text-[10px]">{{ formatDateShort(afterPhoto.workout?.date || afterPhoto.createdAt) }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="beforePhoto && afterPhoto" class="mt-4 flex justify-center gap-3">
      <button @click="shareMode = 'side'; showShareModal = true" class="btn-primary inline-flex items-center gap-2 text-sm">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/>
        </svg>
        Côte à côte
      </button>
      <button @click="shareMode = 'split'; showShareModal = true" class="btn-outline inline-flex items-center gap-2 text-sm">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
