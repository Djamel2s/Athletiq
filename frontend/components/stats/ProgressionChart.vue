<template>
  <div class="card-glass">
    ...
    <div v-if="!selectedExercise" class="text-center py-12">
      <div class="w-14 h-14 bg-primary-100 dark:bg-primary-800 rounded-xl flex items-center justify-center mx-auto mb-4">
        <svg class="w-7 h-7 text-primary-400 dark:text-primary-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <div class="bg-white/30 dark:bg-primary-800/30 rounded-xl p-3 text-center">
          ...
          <p class="text-lg font-bold text-primary-900 dark:text-primary-100">{{ stats.startWeight }} kg</p>
        </div>
        <div class="bg-white/30 dark:bg-primary-800/30 rounded-xl p-3 text-center">
          <p class="text-xs text-primary-500 dark:text-primary-400 mb-1">Poids actuel</p>
          <p class="text-lg font-bold text-primary-900 dark:text-primary-100">{{ stats.currentWeight }} kg</p>
        </div>
        <div class="bg-white/30 dark:bg-primary-800/30 rounded-xl p-3 text-center">
          <p class="text-xs text-primary-500 dark:text-primary-400 mb-1">Progression</p>
          <p class="text-lg font-bold" :class="stats.improvement >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'">
            {{ stats.improvement >= 0 ? '+' : '' }}{{ stats.improvement }}%
          </p>
        </div>
        <div class="bg-white/30 dark:bg-primary-800/30 rounded-xl p-3 text-center">
          <p class="text-xs text-primary-500 dark:text-primary-400 mb-1">Séances</p>
          <p class="text-lg font-bold text-primary-900 dark:text-primary-100">{{ stats.totalSessions }}</p>
        </div>
        <div class="col-span-2 sm:col-span-1 bg-white/30 dark:bg-primary-800/30 rounded-xl p-3 text-center">
          <p class="text-xs text-primary-500 dark:text-primary-400 mb-1">Plateaux</p>
          <p class="text-lg font-bold" :class="stats.plateauCount > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-primary-900 dark:text-primary-100'">
            {{ stats.plateauCount }}
          </p>
        </div>
      </div>
...
const stats = computed(() => {
  ...
  const improvement = startWeight > 0 ? Math.round(((currentWeight - startWeight) / startWeight) * 1000) / 10 : 0
