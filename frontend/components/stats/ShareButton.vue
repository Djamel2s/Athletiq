...
const C = {
  bg1: '#1a1612', bg2: '#2a2318', bg3: '#1f1b16',
  sand: '#d4c4b0', sandLight: '#e8ddd0', sandDark: '#b8a48f', accent: '#9d8569',
  text: '#ffffff', textSoft: '#c8bfb5', textMuted: '#7a7068',
  cardBg: 'rgba(212, 196, 176, 0.06)', cardBorder: 'rgba(212, 196, 176, 0.12)',
}
...
const drawBg = (ctx: CanvasRenderingContext2D) => {
  ...
  grad.addColorStop(0, C.bg1); grad.addColorStop(0.5, C.bg3); grad.addColorStop(1, C.bg2)
  ...
  lg.addColorStop(0, 'rgba(212,196,176,0)'); lg.addColorStop(0.3, C.sandDark)
  lg.addColorStop(0.7, C.sandDark); lg.addColorStop(1, 'rgba(212,196,176,0)')
  ctx.fillStyle = lg; ctx.fillRect(M, 48, W - M * 2, 2)
}

const drawHeader = (ctx: CanvasRenderingContext2D) => {
  ctx.font = `600 42px ${FONT}`; ctx.fillStyle = C.sand; ctx.textAlign = 'center'
  ctx.letterSpacing = '8px'; ctx.fillText('A T H L E T I Q', W / 2, 110); ctx.letterSpacing = '0px'
}

const drawFooter = (ctx: CanvasRenderingContext2D) => {
  const lg = ctx.createLinearGradient(M, 0, W - M, 0)
  lg.addColorStop(0, 'rgba(212,196,176,0)'); lg.addColorStop(0.3, 'rgba(212,196,176,0.3)')
  lg.addColorStop(0.7, 'rgba(212,196,176,0.3)'); lg.addColorStop(1, 'rgba(212,196,176,0)')
  ctx.fillStyle = lg; ctx.fillRect(M, H - 130, W - M * 2, 1)
  if (props.userName) {
    ctx.font = `600 30px ${FONT}`; ctx.fillStyle = C.sand; ctx.textAlign = 'left'
    ctx.fillText(props.userName, M, H - 80)
  }
  ctx.font = `400 28px ${FONT}`; ctx.fillStyle = C.textMuted; ctx.textAlign = 'right'
...
const drawOverview = (ctx: CanvasRenderingContext2D) => {
  const d = props.data
  drawBg(ctx); drawHeader(ctx)

  // Title
  ctx.font = `700 64px ${FONT}`; ctx.fillStyle = C.text; ctx.textAlign = 'center'
  ctx.fillText('Ma Progression', W / 2, 220)

  ctx.font = `400 32px ${FONT}`; ctx.fillStyle = C.textMuted
  ctx.fillText(d.period || 'Depuis le debut', W / 2, 270)

  // Big number - total workouts
  ctx.font = `800 220px ${FONT}`; ctx.fillStyle = C.sand
  ctx.fillText(String(d.totalWorkouts || 0), W / 2, 530)
  ctx.font = `500 44px ${FONT}`; ctx.fillStyle = C.sandDark
  ctx.fillText('ENTRAINEMENTS', W / 2, 590)

  // Separator
  ctx.fillStyle = C.cardBorder; ctx.fillRect(W / 2 - 120, 630, 240, 1)

  // Stats grid 2x2
  const bw = 440, bh = 150, gap = 24
...
const drawTop5 = (ctx: CanvasRenderingContext2D) => {
  ...
  exercises.slice(0, 5).forEach((ex: any, i: number) => {
    ...
    g.addColorStop(0, C.sand); g.addColorStop(1, C.accent)
    ctx.fillStyle = g; ctx.fill()
    ctx.font = `800 32px ${FONT}`; ctx.fillStyle = '#fff'; ctx.textAlign = 'center'
    ctx.fillText(String(i + 1), badgeX + badgeSize / 2, badgeY + badgeSize / 2 + 11)

    // Exercise name
    ctx.font = `600 36px ${FONT}`; ctx.fillStyle = C.text; ctx.textAlign = 'left'
    ctx.fillText(ex.name, badgeX + badgeSize + 28, y + rowH / 2 - 8)

    // Count
    ctx.font = `400 26px ${FONT}`; ctx.fillStyle = C.textMuted
    ctx.fillText(`${ex.count} seances`, badgeX + badgeSize + 28, y + rowH / 2 + 28)

    // Count on right
    ctx.font = `700 40px ${FONT}`; ctx.fillStyle = C.sand; ctx.textAlign = 'right'
...
const drawRecords = (ctx: CanvasRenderingContext2D) => {
  ...
  records.slice(0, 6).forEach((r: any, i: number) => {
    ...
    // Exercise name
    ctx.font = `600 28px ${FONT}`; ctx.fillStyle = C.textSoft; ctx.textAlign = 'left'
    const name = (r.exerciseName || '').length > 18 ? (r.exerciseName || '').substring(0, 18) + '...' : (r.exerciseName || '')
    ...
    if (r.date) {
      ...
      const dateStr = new Intl.DateTimeFormat('fr-FR', { day: 'numeric', month: 'short' }).format(d)
...
const drawGoals = (ctx: CanvasRenderingContext2D) => {
  ...
  ctx.font = `400 30px ${FONT}`; ctx.fillStyle = C.textMuted
  ...
  goals.slice(0, 6).forEach((g: any, i: number) => {
    ...
    ctx.fill(); ctx.strokeStyle = C.cardBorder; ctx.lineWidth = 1; ctx.stroke()

    // Title
    ctx.font = `600 34px ${FONT}`; ctx.fillStyle = C.text; ctx.textAlign = 'left'
    ctx.fillText(g.title || '', M + 30, y + 50)

    // Progress bar
    const barX = M + 30, barY = y + 75, barW = rowW - 60, barH = 16
    roundRect(ctx, barX, barY, barW, barH, 8)
    ctx.fillStyle = 'rgba(212, 196, 176, 0.1)'; ctx.fill()
    ...
    if (progress > 0) {
      ...
      pg.addColorStop(0, C.sand); pg.addColorStop(1, C.accent)
      ctx.fillStyle = pg; ctx.fill()
    }

    // Progress text
    ctx.font = `700 28px ${FONT}`
    ctx.fillStyle = g.achieved ? C.sand : C.textSoft; ctx.textAlign = 'left'
    ctx.fillText(g.achieved ? 'Atteint' : `${Math.round(progress * 100)}%`, M + 30, y + 140)

    // Target value on right
    ctx.font = `400 26px ${FONT}`; ctx.fillStyle = C.textMuted; ctx.textAlign = 'right'
...
const drawProgression = (ctx: CanvasRenderingContext2D) => {
  ...
  if (points.length < 2) {
    ctx.font = `400 36px ${FONT}`; ctx.fillStyle = C.textMuted
    ctx.fillText('Pas assez de donnees', W / 2, H / 2)
    drawFooter(ctx); return
  }

  const chartX = M + 20, chartY = 360, chartW = W - M * 2 - 40, chartH = 800
  const maxW = Math.max(...points.map(p => p.weight))
  const minW = Math.min(...points.map(p => p.weight))
  const range = maxW - minW || 1

  // Y axis labels
  ctx.font = `400 24px ${FONT}`; ctx.fillStyle = C.textMuted; ctx.textAlign = 'right'
  for (let i = 0; i <= 4; i++) {
    const val = Math.round(minW + (range * i) / 4)
    const ly = chartY + chartH - (chartH * i) / 4
    ctx.fillText(`${val}kg`, chartX - 12, ly + 8)
    // Grid line
    ctx.strokeStyle = 'rgba(212, 196, 176, 0.06)'; ctx.lineWidth = 1
  ...
  ag.addColorStop(0, 'rgba(212, 196, 176, 0.2)'); ag.addColorStop(1, 'rgba(212, 196, 176, 0)')
  ...
  ctx.font = `400 22px ${FONT}`; ctx.fillStyle = C.textMuted; ctx.textAlign = 'center'
  if (points[0]?.date) {
    const d1 = new Intl.DateTimeFormat('fr-FR', { day: 'numeric', month: 'short' }).format(new Date(points[0].date))
    ctx.fillText(d1, chartX, chartY + chartH + 40)
  }
  if (points[points.length - 1]?.date) {
    const d2 = new Intl.DateTimeFormat('fr-FR', { day: 'numeric', month: 'short' }).format(new Date(points[points.length - 1].date))
...
const handleShare = async () => {
  ...
  try {
    ...
    switch (props.cardType) {
      case 'overview': drawOverview(ctx); break
      case 'top5': drawTop5(ctx); break
      case 'records': drawRecords(ctx); break
      case 'goals': drawGoals(ctx); break
      case 'progression': drawProgression(ctx); break
    }
