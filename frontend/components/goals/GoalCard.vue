<template>
  <div class="card-glass !p-6 space-y-4">
    ...
    <div class="flex items-start justify-between">
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-2">
          ...
          <span
            v-if="goal.achieved"
            class="px-2.5 py-0.5 rounded-lg text-xs font-semibold bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
          >
            Atteint
          </span>
        </div>
    ...
    <div>
      ...
      <p class="text-xs text-primary-500 dark:text-primary-400 mt-1 text-right">
        {{ goal.progress }}%
      </p>
    </div>
    ...
    <div
      v-if="goal.deadline"
      class="flex items-center gap-1.5 text-sm text-primary-500 dark:text-primary-400"
    >
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path
          stroke-linecap="round"
...
const typeBadgeClass = computed(() => {
  switch (props.goal.type) {
    ...
    default:
      return 'bg-primary-100 text-primary-700 dark:bg-primary-800 dark:text-primary-300';
  }
});
...
const formattedCurrent = computed(() =>
  formatValue(props.goal.currentValue, props.goal.type as GoalType)
);
const formattedTarget = computed(() =>
  formatValue(props.goal.targetValue, props.goal.type as GoalType)
);
...
const formattedDeadline = computed(() => {
  ...
  return new Intl.DateTimeFormat('fr-FR', {
    ...
    month: 'long',
    year: 'numeric',
  }).format(new Date(props.goal.deadline));
});
</script>
