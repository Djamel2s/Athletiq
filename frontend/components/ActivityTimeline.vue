<template>
  <div class="card-glass">
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-xl md:text-3xl font-bold text-primary-900 dark:text-primary-100">
        {{ title }}
      </h2>
      <button
        @click="navigateTo('/workouts')"
        class="text-sm text-primary-500 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-200 font-semibold transition-colors"
      >
        Voir tout →
      </button>
    </div>
    ...
    <div v-else class="space-y-0">
      ...
      >
        ...
        <div class="flex flex-col items-center">
          <div
            class="w-3 h-3 rounded-full bg-gradient-primary flex-shrink-0 mt-1.5 group-hover:scale-125 transition-transform"
          ></div>
          <div
            v-if="index < workouts.length - 1"
            class="w-0.5 flex-1 bg-primary-200 dark:bg-primary-700 my-1"
          ></div>
        </div>
        ...
        <div :class="['flex-1 pb-6', index < workouts.length - 1 ? '' : 'pb-0']">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <p
                class="font-semibold text-primary-900 dark:text-primary-100 group-hover:text-sand-600 transition-colors truncate"
              >
                {{ workout.name }}
              </p>
            ...
            <div
              class="flex items-center gap-3 text-xs text-primary-500 dark:text-primary-400 flex-shrink-0"
            >
              ...
              <span
                ...
                class="flex items-center gap-1 font-semibold text-primary-700 dark:text-primary-300"
              >
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    stroke-linecap="round"
