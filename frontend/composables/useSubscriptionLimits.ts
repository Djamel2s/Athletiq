...
export function useSubscriptionLimits() {
  ...
  const canCreateWorkout = computed(() =>
    isUnlimited(limits.value.workoutsPerWeek) || usage.value.workoutsThisWeek < limits.value.workoutsPerWeek
  )

  const canCreateTemplate = computed(() =>
    isUnlimited(limits.value.workoutTemplates) || usage.value.templates < limits.value.workoutTemplates
  )
  ...
  const photoUsageText = computed(() =>
    formatUsage(usage.value.photos, limits.value.photos)
  )

  const goalUsageText = computed(() =>
    formatUsage(usage.value.goals, limits.value.goals)
  )
