...
export const useSessionApi = () => {
  ...
  const joinSessionLocal = (code: string, email: string, password: string) => apiFetch(`/sessions/${code}/join-local`, { method: 'POST', body: { email, password } })
  ...
  const setWorkout = (id: number, workoutId: number) => apiFetch(`/sessions/${id}/set-workout`, { method: 'POST', body: { workoutId } })

  return { createSession, joinSession, joinSessionLocal, getSession, startSession, pauseSession, resumeSession, leaveSession, endSession, setWorkout }
}
