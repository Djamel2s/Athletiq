...
export const useBodyApi = () => {
  ...
  const createBodyStat = async (data: {
    weight: number;
    bodyFat?: number;
    notes?: string;
    date?: string;
  ...
  const getTimelapse = async (startDate?: string, endDate?: string) => {
    ...
    const res = await $fetch<{ photos: ProgressPhoto[]; total: number }>(
      `${config.public.apiUrl}/photos/timelapse${query}`,
      fetchOpts()
    );
    return res.photos;
  };
  ...
  const getRecentPhotos = async (limit = 10) => {
    return await $fetch<ProgressPhoto[]>(
      `${config.public.apiUrl}/photos/recent?limit=${limit}`,
      fetchOpts()
    );
  };
