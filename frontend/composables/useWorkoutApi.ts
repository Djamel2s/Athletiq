...
export const useWorkoutApi = () => {
  ...
  const updateWorkout = async (id: number, data: Partial<Workout>) => {
    return await apiFetch<{ message: string; workout: Workout }>(`/workouts/${id}`, {
      method: 'PUT',
      timeout: API_TIMEOUT,
      body: data,
    });
  ...
  const startWorkout = async (id: number) => {
    return await apiFetch<{ message: string; workout: Workout }>(`/workouts/${id}/start`, {
      method: 'POST',
      timeout: API_TIMEOUT,
    });
  };
  ...
  const completeWorkout = async (id: number) => {
    return await apiFetch<{ message: string; workout: Workout }>(`/workouts/${id}/complete`, {
      method: 'POST',
      timeout: API_TIMEOUT,
    });
  };
  ...
  const getExercisesByMuscleGroup = async (muscleGroup: string) => {
    return await apiFetch<ExerciseLibrary[]>(`/exercises/muscle/${muscleGroup}`, {
      timeout: API_TIMEOUT,
    });
  };
  ...
  const addExerciseToWorkout = async (
    ...
    return await apiFetch<Exercise>(`/workouts/${workoutId}/exercises`, {
      method: 'POST',
      timeout: API_TIMEOUT,
      body: data,
    });
  ...
  const updateExercise = async (workoutId: number, exerciseId: number, data: Partial<Exercise>) => {
  ...
  const deleteExercise = async (workoutId: number, exerciseId: number) => {
    return await apiFetch<{ message: string }>(`/workouts/${workoutId}/exercises/${exerciseId}`, {
      method: 'DELETE',
      timeout: API_TIMEOUT,
    });
  };
  ...
  const addSetToExercise = async (
    ...
    return await apiFetch<Set>(`/workouts/${workoutId}/exercises/${exerciseId}/sets`, {
      method: 'POST',
      timeout: API_TIMEOUT,
      body: data,
    });
