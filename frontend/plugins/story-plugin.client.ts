...
export default defineNuxtPlugin(() => {
  return {
    provide: {
      shareToStoryNative: async (options: { blob?: Blob; fileUri?: string; url?: string; attribution?: string }) => {
        try {
          ...
          const res = await (Plugins as any).StoryPlugin.shareToStory({ fileUri, url: options.url, attribution: options.attribution })
