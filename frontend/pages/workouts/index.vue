    ...
    <div class="px-4 md:px-6 pb-28 lg:pb-20 max-w-7xl mx-auto">
      ...
      <div class="mb-8 slide-up">
        <div class="flex justify-center">
          <div
            class="flex space-x-1 bg-white/60 dark:bg-primary-800/60 backdrop-blur-lg rounded-xl p-1"
          >
            <button
      ...
      <!-- Mes workouts -->
      <div
        v-if="!workoutStore.isLoading && !workoutStore.error && activeTab === 'workouts'"
        class="slide-up"
      >
        <div v-if="workoutStore.templates.length === 0" class="card-glass text-center py-16">
          <svg
            class="w-20 h-20 mx-auto mb-6 text-primary-300"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          >
            ...
            <div class="p-5">
              <div class="flex items-start justify-between mb-3">
                <h3
                  class="text-lg font-bold text-primary-900 dark:text-primary-100 group-hover:text-sand-700 dark:group-hover:text-sand-400 transition-colors leading-tight flex-1 mr-2"
                >
                  {{ workout.name }}
                </h3>
              ...
              <p
                ...
                class="text-sm text-primary-500 dark:text-primary-400 mb-4 line-clamp-2"
              >
                {{ workout.description }}
              </p>
              ...
              <div class="flex items-center gap-2">
                <span
                  class="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-lg bg-sand-500/10 dark:bg-sand-600/10 text-sand-700 dark:text-sand-400"
                >
      ...
      <!-- Historique -->
      <div
        v-if="!workoutStore.isLoading && !workoutStore.error && activeTab === 'history'"
        class="space-y-6 slide-up"
      >
        ...
        >
          <div class="p-4 md:p-5">
            <div class="flex items-start gap-3 md:gap-4">
              <!-- Date badge -->
              <div
                class="flex-shrink-0 w-14 h-14 rounded-xl bg-primary-50 dark:bg-primary-800 border border-primary-200/60 dark:border-primary-700/60 flex flex-col items-center justify-center text-center"
              >
                <span
                  class="text-xs font-medium text-primary-500 dark:text-primary-400 uppercase leading-none"
                  >{{ getMonthShort(workout.completedAt!) }}</span
                >
                <span
                  class="text-lg font-bold text-primary-900 dark:text-primary-100 leading-none mt-0.5"
                  >{{ getDay(workout.completedAt!) }}</span
                >
              </div>
              ...
              <div class="flex-1 min-w-0">
                <h3
                  class="text-base md:text-lg font-bold text-primary-900 dark:text-primary-100 group-hover:text-sand-700 dark:group-hover:text-sand-400 transition-colors truncate"
                >
                  {{ workout.name }}
                </h3>
                <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
                  ...
                  <span
                    v-if="workout.duration"
                    class="inline-flex items-center gap-1 text-xs font-semibold text-sand-600 dark:text-sand-400"
                  >
      ...
      >
        ...
        >
          ...
          <div v-else class="space-y-2 mb-5 max-h-40 overflow-y-auto">
            ...
            >
              <div
                class="w-8 h-8 rounded-full bg-gradient-primary flex items-center justify-center flex-shrink-0 overflow-hidden"
              >
                <img
              ...
              <div
                ...
                class="w-5 h-5 rounded-full bg-sand-500 flex items-center justify-center flex-shrink-0"
              >
                <svg
...
const shareTemplate = async (id: number) => {
  ...
  sharingId.value = id;
  try {
    const { shareToken } = await apiFetch<{ shareToken: string }>(`/share/${id}`, {
      method: 'POST',
    });
