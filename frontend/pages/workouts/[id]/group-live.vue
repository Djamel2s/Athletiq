<template>
  <div class="min-h-screen">
    ...
    <div
      v-if="showCountdown"
      class="fixed inset-0 bg-white dark:bg-primary-900 z-50 flex items-center justify-center overflow-hidden"
    >
      <div
        class="absolute inset-0 bg-gradient-to-br from-sand-500/5 via-transparent to-sand-600/10 pointer-events-none"
      ></div>
      <div class="text-center relative">
        <div
          class="text-8xl md:text-[12rem] font-bold bg-gradient-to-b from-sand-500 to-sand-700 dark:from-sand-400 dark:to-sand-600 bg-clip-text text-transparent animate-pulse font-mono"
        >
          {{ countdownNumber }}
        </div>
        <p class="text-2xl text-primary-600 dark:text-primary-400 mt-6 font-medium">
          Preparez-vous...
        </p>
      </div>
    </div>
      ...
      <div class="px-4 pb-3">
        <div class="flex gap-2 overflow-x-auto">
          ...
          >
            <!-- Avatar -->
            <div
              class="w-7 h-7 rounded-full bg-gradient-primary flex items-center justify-center flex-shrink-0 overflow-hidden"
            >
              <img
            ...
            <div class="min-w-0">
              <p
                class="text-[11px] font-semibold text-primary-900 dark:text-primary-100 truncate max-w-[60px]"
              >
              ...
              >
                {{
                  participant.restRemaining <= 0
                    ? 'Pret'
                    : formatRestTime(participant.restRemaining)
                }}
              </p>
            </div>
            ...
            <!-- Finished indicator -->
            <div
              v-if="participant.finished"
              class="w-4 h-4 rounded-full bg-green-500 flex items-center justify-center flex-shrink-0"
            >
              <svg
    ...
    <div class="pt-36 pb-20 px-4 max-w-3xl mx-auto">
      ...
      <div v-else class="space-y-4 slide-up">
        ...
        >
          <div
            v-if="isMyTurn"
            class="absolute inset-0 bg-gradient-to-r from-sand-500/10 via-sand-500/5 to-sand-600/10 pointer-events-none"
          ></div>
          <div class="relative">
            <p
              v-if="isMyTurn"
              class="text-2xl font-bold bg-gradient-to-r from-sand-500 to-sand-700 dark:from-sand-400 dark:to-sand-600 bg-clip-text text-transparent"
            >
              C'est ton tour !
            </p>
        ...
        <div v-if="!isMyTurn && !isLocalMode" class="card-glass text-center py-8 space-y-4">
          <div
            class="w-16 h-16 mx-auto bg-primary-100 dark:bg-primary-800 rounded-full flex items-center justify-center"
          >
          ...
          <div v-if="myRestRemaining > 0" class="mt-4">
            <p class="text-xs text-primary-400 dark:text-primary-500 uppercase tracking-wider mb-2">
              Ton repos
            </p>
          ...
          <div v-else class="mt-4">
            <span
              class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-green-500/10 text-green-700 dark:text-green-400 text-sm font-semibold border border-green-500/20"
            >
        ...
        <div v-if="isMyTurn || isLocalMode">
          ...
          <Transition name="fade">
            ...
            >
              <p
                class="text-sm text-primary-500 dark:text-primary-400 mb-2 tracking-widest uppercase"
              >
                Repos
              </p>
                ...
                <svg class="transform -rotate-90" width="240" height="240">
                  ...
                  <defs>
                    <linearGradient id="groupRestGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                      ...
                      <stop
                        offset="100%"
                        style="stop-color: rgb(var(--sand-600)); stop-opacity: 1"
                      />
                    </linearGradient>
                  </defs>
                </svg>
          ...
          <div v-if="!showRestTimer">
            ...
            <div class="text-center mb-6">
              <p class="text-sm text-primary-600 dark:text-primary-400 mb-2">
                {{ isLocalMode ? activeTurnParticipant?.name + ' - ' : '' }}Exercice
                {{ currentExerciseIndex + 1 }} / {{ workout.exercises?.length || 0 }} · Serie
            ...
            <div class="card-glass space-y-6">
              ...
              <div class="flex gap-3">
                <button @click="skipCurrentSet" class="btn-outline py-4 px-5 text-sm">
                  Passer
                </button>
    ...
    <Transition name="fade">
      <div
        v-if="isPaused"
        class="fixed inset-0 z-[60] bg-black/60 backdrop-blur-md flex items-center justify-center"
      >
        <div class="card-glass max-w-sm w-full mx-4 text-center py-10 px-6">
          <div
            class="w-16 h-16 mx-auto mb-4 bg-amber-500/10 rounded-full flex items-center justify-center border border-amber-500/20"
          >
          ...
          <p class="text-primary-600 dark:text-primary-400 mb-6">
            {{ pauseReason || "L'hote a mis la session en pause" }}
          </p>
          <button v-if="isHost" @click="handleResume" class="btn-primary w-full py-3">
            Reprendre
          </button>
        </div>
      </div>
    ...
    <Transition name="fade">
      <div v-if="showDisconnectWarning" class="fixed top-20 left-4 right-4 z-[70] max-w-md mx-auto">
        <div
          class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-2xl p-4 flex items-center gap-3 shadow-lg"
        >
          <div
            class="w-8 h-8 bg-red-500/10 rounded-full flex items-center justify-center flex-shrink-0 border border-red-500/20"
          >
      ...
      >
        <div class="min-h-full flex items-center justify-center py-12 px-4">
          <div class="max-w-md w-full text-center space-y-8 fade-in">
            <div
              class="w-24 h-24 mx-auto bg-gradient-primary rounded-full flex items-center justify-center shadow-lg"
            >
              ...
              >
                ...
                <div class="flex-1 min-w-0 text-left">
                  ...
                  <p class="text-xs text-primary-500 dark:text-primary-400">
                    {{ participant.setsCompleted || 0 }} series completees
                  </p>
                </div>
                <div
                  class="w-7 h-7 rounded-full bg-green-500/10 flex items-center justify-center flex-shrink-0 border border-green-500/20"
                >
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          <div class="text-center">
            <div
              class="w-14 h-14 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-red-500/20"
            >
            ...
            <p class="text-primary-600 dark:text-primary-400 text-sm mb-6">
              Les autres participants continueront sans toi.
            </p>
            <div class="space-y-3">
              <button
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          <div class="text-center">
            <div
              class="w-14 h-14 bg-green-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-green-500/20"
            >
            ...
            <p class="text-primary-600 dark:text-primary-400 text-sm mb-6">
              La session sera terminee pour tous les participants.
            </p>
            <div class="space-y-3">
              <button
                @click="handleEndSession"
                :disabled="isEnding"
                class="w-full py-3 rounded-2xl bg-gradient-primary text-white font-medium transition-colors disabled:opacity-50"
              >
...
const progress = computed(() => {
  ...
  const totalSetsAll = workout.value.exercises.reduce(
    (sum: number, ex: any) => sum + (ex.targetSets || 3),
    0
  );
  const completedSetsCount =
    workout.value.exercises
      .slice(0, currentExerciseIndex.value)
      .reduce((sum: number, ex: any) => sum + (ex.targetSets || 3), 0) +
