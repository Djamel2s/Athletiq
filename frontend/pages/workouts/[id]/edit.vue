            ...
            <template v-for="(exercise, index) in selectedExercises" :key="exercise.id">
              ...
              <div
                v-if="
                  ...
                  (index === 0 ||
                    selectedExercises[index - 1]?.supersetGroup !== exercise.supersetGroup)
                "
                class="flex items-center gap-2 mb-1 mt-4 first:mt-0"
              >
                <span
                  class="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1 rounded-full bg-sand-500/15 text-sand-700 dark:text-sand-400 border border-sand-500/30 uppercase tracking-wider"
                >
...
const toggleSupersetLink = (index: number) => {
  ...
  if (areSupersetLinked(index)) {
    ...
    const aConnectedAbove =
      index > 0 && selectedExercises.value[index - 1]?.supersetGroup === group;
    if (!aConnectedAbove) {
      a.supersetGroup = undefined;
    }
    // For b and everything after it that shares this group, assign a new group or null
    const bConnectedBelow =
      index + 2 < selectedExercises.value.length &&
      selectedExercises.value[index + 2]?.supersetGroup === group;
    if (!bConnectedBelow) {
