<template>
  <div class="min-h-screen overflow-x-hidden">
    ...
    <div v-else class="pb-28 lg:pb-20 max-w-7xl mx-auto">
      <div class="flex px-4 md:px-4">
        ...
        <div class="hidden lg:block lg:mr-8 flex-shrink-0 self-start sticky top-2">
          <div
            class="group bg-white/70 dark:bg-primary-800/70 backdrop-blur-md rounded-xl border border-primary-200 dark:border-primary-700 hover:shadow-lg hover:border-primary-400 dark:hover:border-primary-500 p-4 flex flex-col w-[72px] hover:w-[200px] overflow-hidden"
            style="
          ...
          >
            ...
            <div class="flex flex-col space-y-6">
              ...
              <!-- Feed removed from dashboard sidebar per UX change -->
            </div>
        ...
        <div class="flex-1 min-w-0">
          ...
          <!-- Comeback Banner -->
          <ComebackBanner
            v-if="lastWorkoutDate"
            :last-workout-date="lastWorkoutDate"
          ...
          <div
            ...
            class="mb-6 md:mb-8 fade-in cursor-pointer"
            @click="navigateTo('/streak')"
          >
            <div
              class="flex items-center gap-4 px-5 py-4 rounded-2xl bg-gradient-to-r from-sand-500/10 to-sand-600/10 dark:from-sand-500/15 dark:to-sand-600/15 border border-sand-500/30 dark:border-sand-600/20 hover:shadow-lg hover:scale-[1.01] transition-all"
            >
              <div
                class="w-12 h-12 bg-gradient-primary rounded-xl flex items-center justify-center flex-shrink-0"
              >
          ...
          <!-- Actions rapides -->
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8 mb-8 md:mb-12 slide-up">
          ...
          <div class="mb-8 md:mb-12 slide-up">
            <LazyDashboardMuscleRecoveryFigure
            ...
            <div class="space-y-3">
              ...
              >
                <div class="flex items-start gap-3">
                  ...
                  >
                    <img
                      v-if="plannedBro(pw).avatarUrl"
                      :src="plannedBro(pw).avatarUrl"
                      class="w-full h-full object-cover"
                    />
                    <span v-else class="text-xs font-bold text-white">{{
                      (
                        plannedBro(pw).firstName?.[0] ||
                        plannedBro(pw).username?.[0] ||
                        '?'
                      ).toUpperCase()
                    }}</span>
                  </div>
                  ...
                  <div class="flex-1 min-w-0">
                    <h3 class="text-base font-bold text-primary-900 dark:text-primary-100 truncate">
                      {{ pw.name }}
                    </h3>
                    <p class="text-sm text-primary-500 dark:text-primary-400">
                      avec
                      {{
                        plannedBro(pw).username ||
                        `${plannedBro(pw).firstName || ''} ${plannedBro(pw).lastName || ''}`.trim()
                      }}
                    </p>
                    ...
                    <p
                      v-if="pw.notes"
                      class="text-xs text-primary-400 dark:text-primary-500 mt-0.5 italic"
                    >
                      {{ pw.notes }}
                    </p>
                  </div>
    ...
    <MobileBottomNav active-path="/dashboard" />
  </div>
</template>
...
const recoveryData = ref<{
  score: number;
  muscleRecovery: Array<{ muscle: string; score: number; daysSince: number; lastVolume: number }>;
...
const lastWorkoutDate = computed(() => {
  if (completedWorkouts.value.length === 0) return null;
  return completedWorkouts.value[0]?.completedAt || null;
});
