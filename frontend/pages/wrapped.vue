    ...
    <div class="px-4 md:px-6 pb-28 lg:pb-20 max-w-5xl mx-auto">
      ...
      <template v-else>
        ...
        <div
          ...
          class="card-glass text-center py-20 max-w-2xl mx-auto fade-in"
        >
          <svg
          ...
          <p class="text-lg text-primary-600 dark:text-primary-400">
            Complete des seances pour voir ton bilan !
          </p>
        </div>
        ...
        <template v-if="filteredWorkouts.length > 0">
          ...
          <div class="text-center mb-8 fade-in">
            ...
            <h2
              class="text-6xl md:text-8xl font-bold text-primary-900 dark:text-primary-100 mb-2 transition-all duration-700"
            >
              {{ totalWorkouts }}
            </h2>
          ...
          <div v-if="topExercises.length" class="card-glass mb-8 slide-up">
            ...
            <div class="flex flex-wrap gap-4">
              ...
              >
                <div
                  class="w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-primary flex-shrink-0"
                >
                  <span class="text-white font-bold text-lg">{{ i + 1 }}</span>
                </div>
                <div class="flex-1 min-w-0">
                  <p class="text-lg font-bold text-primary-900 dark:text-primary-100 truncate">
                    {{ ex.name }}
                  </p>
                  <p class="text-sm text-primary-500 dark:text-primary-400">{{ ex.count }} fois</p>
                </div>
                <div class="text-right flex-shrink-0">
                  <p class="text-sm font-semibold text-sand-700 dark:text-sand-500">
          ...
          <div v-if="muscleDistribution.length" class="card-glass mb-8 slide-up">
            ...
            <div class="space-y-3">
              <div
                v-for="muscle in muscleDistribution"
                :key="muscle.name"
                class="flex items-center gap-3"
              >
                <span
                  class="w-24 text-sm text-primary-600 dark:text-primary-400 text-right flex-shrink-0 truncate"
                  >{{ muscle.label }}</span
                >
                <div
                  class="flex-1 h-6 bg-primary-200 dark:bg-primary-800 rounded-full overflow-hidden"
                >
                  <div
                ...
                <span
                  class="w-12 text-sm font-semibold text-primary-900 dark:text-primary-100 text-right"
                  >{{ muscle.percent }}%</span
                >
              </div>
            </div>
              ...
              >
                ...
                <div class="text-right flex-shrink-0">
                  <p class="text-sm font-semibold text-primary-500 dark:text-primary-400">
                    ...
                    <span class="text-sand-700 dark:text-sand-500 font-bold"
                      >{{ imp.after }} kg</span
                    >
                  </p>
    ...
    <Teleport to="body">
      <div v-if="showShareModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        >
          ...
          <ShareCard type="wrapped" title="Mon Bilan Athletiq" :data="shareData" />
        </div>
      </div>
    </Teleport>
...
const totalCalories = computed(() => Math.round((totalDurationSeconds.value / 60) * 6));
...
const shareData = computed(() => ({
  ...
  topExercise: topExercises.value[0]
    ? {
        name: topExercises.value[0].name,
        count: topExercises.value[0].count,
        pr: topExercises.value[0].maxWeight > 0 ? `${topExercises.value[0].maxWeight}kg` : '---',
      }
    : null,
