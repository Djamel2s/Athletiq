<template>
  <div class="min-h-screen">
    ...
    <div class="px-4 md:px-6 pb-28 lg:pb-20 max-w-5xl mx-auto">
      <!-- Page Header -->
      <div class="fade-in text-center mb-8">
        ...
        <p class="text-lg text-primary-600 dark:text-primary-400">{{ t('streak.pageSubtitle') }}</p>
      </div>
      ...
      <template v-else>
        ...
        <div class="text-center mb-8 fade-in">
          ...
          >
            <div class="text-center animate-bounce">
              <div
                class="w-32 h-32 md:w-48 md:h-48 bg-gradient-primary rounded-full flex items-center justify-center mx-auto mb-4"
              >
                <svg
              ...
              <p class="text-3xl md:text-5xl font-bold text-primary-900 dark:text-primary-100">
          ...
          <div class="relative inline-block mb-4">
            <div
              class="w-20 h-20 md:w-28 md:h-28 bg-gradient-primary rounded-3xl flex items-center justify-center"
            >
              <svg
            ...
            <div
              v-if="streakData && streakData.currentStreak > 0"
              class="absolute -top-2 -right-4 w-10 h-10 bg-sand-600 rounded-full flex items-center justify-center text-white font-bold text-sm animate-pulse"
            >
          ...
          <h2 class="text-4xl md:text-6xl font-bold text-primary-900 dark:text-primary-100 mb-2">
            ...
            <span class="text-2xl md:text-3xl font-normal text-primary-500">
              {{
                t('streak.weekUnit', { plural: (streakData?.currentStreak || 0) > 1 ? 's' : '' })
              }}
            </span>
          </h2>
          ...
          <div v-if="streakData" class="max-w-md mx-auto mt-6">
            ...
            >
              ...
              <span class="font-semibold"
                >{{ streakData.currentWeekWorkouts }} / {{ streakData.streakGoalPerWeek }}</span
              >
            </div>
            <div class="h-3 bg-primary-200 dark:bg-primary-800 rounded-full overflow-hidden">
              <div class="h-full rounded-full transition-all duration-700"></div>
            </div>
            <p
              v-if="weekProgress >= 100"
              class="text-sm text-sand-700 dark:text-sand-500 mt-2 font-semibold"
            >
              {{ t('streak.goalReachedWeek') }}
            </p>
            <p v-else class="text-sm text-primary-500 dark:text-primary-400 mt-2">
              {{
                t('streak.remainingSessions', {
                  count: streakData.streakGoalPerWeek - streakData.currentWeekWorkouts,
                  plural:
                    streakData.streakGoalPerWeek - streakData.currentWeekWorkouts > 1 ? 's' : '',
                })
              }}
            </p>
          </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8 slide-up">
          ...
          <div class="card-glass text-center">
            <p class="text-3xl md:text-4xl font-bold text-primary-900 dark:text-primary-100">
              {{ streakData?.daysSinceLastWorkout ?? '—' }}
            </p>
            <p class="text-xs md:text-sm text-primary-500 dark:text-primary-400 mt-1">
              {{ t('streak.daysSinceLast') }}
            </p>
          </div>
        </div>
            ...
            >
              <div
                :class="[
                  ...
                  m.achieved ? 'bg-gradient-primary' : 'bg-primary-200 dark:bg-primary-700',
                ]"
              >
                <svg
                  v-if="m.achieved"
              ...
              <span
                v-if="m.achieved"
                class="text-xs text-sand-700 dark:text-sand-500 font-medium mt-1"
                >{{ t('streak.achieved') }}</span
              >
              <span v-else class="text-xs text-primary-400 mt-1">{{
                t('streak.remaining', { count: m.weeks - (streakData?.currentStreak || 0) })
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="streakData?.weeklyHistory?.length" class="card-glass mb-8 slide-up">
          <h3 class="text-xl font-bold text-primary-900 dark:text-primary-100 mb-6">
            {{ t('streak.weeklyActivity') }}
          </h3>
          <div class="flex flex-wrap gap-2 justify-center">
            <div v-for="week in streakData.weeklyHistory" :key="week.week" class="group relative">
              <div
                :class="[
              ...
              <!-- Tooltip -->
              <div
                class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-primary-900 dark:bg-primary-100 text-white dark:text-primary-900 text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-10"
              >
                {{ week.week }}
              </div>
            </div>
          ...
          <div
            class="flex items-center justify-center gap-4 mt-4 text-xs text-primary-500 dark:text-primary-400"
          >
            ...
            >
              <div
                :class="[
                  ...
                  week.workoutCount >= 3
                    ? 'bg-gradient-to-t from-sand-600 to-sand-500'
                    : 'bg-primary-300 dark:bg-primary-600',
                ]"
                :style="{ height: `${Math.max((week.avgMaxWeight / maxWeight) * 100, 4)}%` }"
              ></div>
              <span class="text-[10px] text-primary-400 whitespace-nowrap"
                >{{ week.workoutCount }}x</span
              >
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 mt-6">
            <div class="p-4 rounded-xl bg-primary-100/50 dark:bg-primary-800/30 text-center">
              <p class="text-xs text-primary-500 dark:text-primary-400 mb-1">
                {{ t('streak.weeksLess3x') }}
              </p>
              <p class="text-2xl font-bold text-primary-700 dark:text-primary-300">
                {{ correlationData.lowFreqAvgProgress.toFixed(1) }}%
              </p>
              <p class="text-xs text-primary-400">{{ t('streak.avgProgress') }}</p>
            </div>
            <div
              class="p-4 rounded-xl bg-gradient-to-br from-sand-500/20 to-sand-600/20 border border-sand-500/30 text-center"
            >
              <p class="text-xs text-primary-500 dark:text-primary-400 mb-1">
                {{ t('streak.weeks3xPlus') }}
              </p>
              <p class="text-2xl font-bold text-primary-900 dark:text-primary-100">
                {{ correlationData.highFreqAvgProgress.toFixed(1) }}%
              </p>
              <p class="text-xs text-primary-400">{{ t('streak.avgProgress') }}</p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1">
              <h3 class="text-lg font-bold text-primary-900 dark:text-primary-100">
                {{ t('streak.weeklyRecap') }}
              </h3>
              <p class="text-sm text-primary-600 dark:text-primary-400">
                {{ t('streak.weeklyRecapDesc') }}
              </p>
            </div>
        ...
        <div class="card-glass mb-8 slide-up">
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-primary-500 dark:text-primary-400 ml-2">{{
              t('streak.perWeek')
            }}</span>
          </div>
        </div>
      ...
      <div v-if="showShareModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        >
          ...
          <h3 class="text-xl font-bold text-primary-900 dark:text-primary-100 mb-4">
            {{ showShareModal === 'streak' ? t('streak.share') : t('streak.weeklyRecap') }}
          </h3>
          ...
          <div v-if="showShareModal === 'recap' && !recapData" class="text-center py-8">
            ...
            ></div>
            <p class="text-primary-500">{{ t('streak.loadingRecap') }}</p>
          </div>
        </div>
      </div>
    </Teleport>
...
watch(showShareModal, async (val) => {
  if (val === 'recap' && !recapData.value) {
    try {
      ...
      toast.error(t('common.error'), t('streak.errorLoadRecap'));
    }
  }
});
...
const completedWorkouts = computed(() => workoutStore.workouts.filter((w) => w.completedAt));
...
const getMotivationMessage = () => {
  ...
  if (s >= 52) return t('streak.motivation52');
  if (s >= 26) return t('streak.motivation26');
  if (s >= 12) return t('streak.motivation12');
  if (s >= 8) return t('streak.motivation8');
  if (s >= 4) return t('streak.motivation4');
  if (s >= 2) return t('streak.motivation2');
  return t('streak.motivation0');
};
...
const updateStreakGoal = async (n: number) => {
  try {
    ...
    toast.success(t('streak.goalUpdated'), t('streak.perWeekValue', { n }));
  } catch {
    toast.error(t('common.error'), t('streak.errorUpdateGoal'));
  }
};
