            ...
            <div v-if="posts.length > 0" class="grid grid-cols-3 gap-4">
              <div v-for="post in posts" :key="post.id" class="relative rounded-lg overflow-hidden bg-primary-900/40" @click="openPostModal(post)" style="cursor:pointer;">
                <!-- Three-dots menu (top-right) -->
                  ...
                  <div v-if="openMenuPostId === post.id" class="absolute right-0 mt-2 w-36 rounded-lg shadow-lg bg-white dark:bg-primary-900 border border-primary-200 dark:border-primary-800 overflow-hidden z-20">
                    <button @click.stop.prevent="sharePost(post)" class="w-full py-2 flex items-center justify-center text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-800 transition-colors" title="Partager">
                      <Icon name="lucide:share-2" class="w-4 h-4" />
                      <span class="sr-only">Partager</span>
                    </button>
                    <button v-if="isOwnProfile" @click.stop.prevent="(async ()=>{ await deletePostAction(post.id) })()" class="w-full py-2 flex items-center justify-center text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors" title="Supprimer">
