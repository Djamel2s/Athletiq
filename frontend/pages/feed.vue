<template>
  <div class="min-h-screen geometric-bg">
    ...
    <div class="pt-20 md:pt-28 px-4 md:px-6 pb-28 lg:pb-20 max-w-lg mx-auto">
      ...
      <div v-else-if="posts.length > 0" class="space-y-4 mb-8 slide-up">
        <div v-for="post in posts" :key="post.id" class="card-glass !p-4">
          ...
          <div v-if="post.type === 'WORKOUT_COMPLETED'" class="mb-3">
            <p class="text-sm text-primary-700 dark:text-primary-300 mb-2">
              A termine <span class="font-semibold">{{ post.data?.workoutName || 'un workout' }}</span>
            </p>
            <div class="flex gap-3 flex-wrap">
              <span v-if="post.data?.duration" class="inline-flex items-center gap-1.5 text-xs bg-primary-100 dark:bg-primary-800 px-2.5 py-1 rounded-lg">
                <Icon name="lucide:clock" class="w-3.5 h-3.5 text-primary-500" />
                <span class="text-primary-700 dark:text-primary-300">{{ post.data.duration }}min</span>
              </span>
              <span v-if="post.data?.exercises" class="inline-flex items-center gap-1.5 text-xs bg-primary-100 dark:bg-primary-800 px-2.5 py-1 rounded-lg">
                <Icon name="lucide:dumbbell" class="w-3.5 h-3.5 text-primary-500" />
                <span class="text-primary-700 dark:text-primary-300">{{ post.data.exercises }} exercices</span>
              </span>
              <span v-if="post.data?.volume" class="inline-flex items-center gap-1.5 text-xs bg-primary-100 dark:bg-primary-800 px-2.5 py-1 rounded-lg">
                <Icon name="lucide:weight" class="w-3.5 h-3.5 text-primary-500" />
                <span class="text-primary-700 dark:text-primary-300">{{ formatVolume(post.data.volume) }}kg</span>
              </span>
            </div>
          </div>
          ...
          <div v-else-if="post.type === 'PR_ACHIEVED'" class="mb-3">
            <div class="flex items-center gap-2 mb-1">
              ...
              <p class="text-sm font-semibold text-primary-900 dark:text-primary-100">Nouveau record !</p>
            </div>
            <p class="text-sm text-primary-700 dark:text-primary-300">
              {{ post.data?.exerciseName }} : <span class="font-bold text-sand-600 dark:text-sand-400">{{ post.data?.weight }}kg</span>
            </p>
          </div>
          ...
          <div v-else-if="post.type === 'TEMPLATE_SHARED'" class="mb-3">
            ...
            <div class="card-glass !p-3 !bg-primary-50/50 dark:!bg-primary-800/30">
              <p class="text-sm font-semibold text-primary-900 dark:text-primary-100">{{ post.data?.templateName }}</p>
              <p v-if="post.data?.description" class="text-xs text-primary-500 dark:text-primary-400 mt-1">{{ post.data.description }}</p>
              <button class="mt-2 text-xs font-medium text-sand-600 dark:text-sand-400 hover:underline inline-flex items-center gap-1">
          ...
          <div v-else class="mb-3">
            <p class="text-sm text-primary-700 dark:text-primary-300">{{ post.data?.text || post.type }}</p>
          </div>
        ...
        <div v-if="hasMore" class="text-center py-4">
          ...
          <div v-else class="inline-block animate-spin rounded-full h-8 w-8 border-2 border-primary-300 dark:border-primary-600 border-t-primary-600 dark:border-t-primary-400"></div>
        </div>
      </div>

      <!-- Empty state -->
      <div v-else class="text-center py-16 slide-up">
        <Icon name="lucide:users" class="w-20 h-20 mx-auto mb-4 text-primary-300 dark:text-primary-600" />
        <h3 class="text-lg font-semibold text-primary-900 dark:text-primary-100 mb-2">Ton feed est vide</h3>
        <p class="text-sm text-primary-500 dark:text-primary-400 mb-6">Ajoute des Gym Bros pour voir leur activite</p>
        <button @click="showSearch = true" class="btn-primary px-6 py-2.5 text-sm font-semibold inline-flex items-center gap-2">
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="showSearch" class="fixed inset-0 z-[90] flex items-start justify-center pt-20 px-4" @click.self="showSearch = false">
          <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>
          <div class="relative w-full max-w-md bg-white dark:bg-primary-900 rounded-2xl shadow-2xl overflow-hidden">
            <!-- Search input -->
            <div class="p-4 border-b border-primary-100 dark:border-primary-800">
              <div class="flex items-center gap-3">
                ...
                <input
                  ...
                  autofocus
                />
                <button @click="showSearch = false" class="text-primary-400 hover:text-primary-600 dark:hover:text-primary-300">
            ...
            <div class="max-h-[60vh] overflow-y-auto">
              <div v-if="searching" class="p-8 text-center">
                <div class="inline-block animate-spin rounded-full h-6 w-6 border-2 border-primary-300 border-t-primary-600"></div>
              </div>
              <div v-else-if="searchResults.length > 0" class="py-2">
                ...
                >
                  <div class="w-10 h-10 rounded-full overflow-hidden flex-shrink-0" :class="user.avatarUrl ? '' : 'bg-gradient-primary flex items-center justify-center'">
                    <img v-if="user.avatarUrl" :src="user.avatarUrl" alt="" class="w-full h-full object-cover" />
                    <span v-else class="text-white text-sm font-bold">{{ (user.firstName?.charAt(0) || '').toUpperCase() }}</span>
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-semibold text-primary-900 dark:text-primary-100 truncate">{{ user.firstName }} {{ user.lastName }}</p>
                    <p class="text-xs text-primary-500 dark:text-primary-400">@{{ user.username }}</p>
                  </div>
              ...
              <div v-else-if="searchQuery.length >= 2" class="p-8 text-center">
                <p class="text-sm text-primary-500 dark:text-primary-400">Aucun utilisateur trouve</p>
              </div>
              <div v-else class="p-8 text-center">
                <p class="text-sm text-primary-400 dark:text-primary-500">Tape au moins 2 caracteres</p>
              </div>
            </div>
          </div>
        </div>
