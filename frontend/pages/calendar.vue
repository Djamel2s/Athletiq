      ...
      <template v-else>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8 fade-in">
          ...
          <div class="card-glass text-center">
            <div
              class="w-12 h-12 mx-auto mb-3 bg-gradient-primary rounded-xl flex items-center justify-center"
            >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-6 md:mb-8 slide-up">
          ...
          <div class="lg:col-span-2">
            ...
            <div v-else class="card-glass flex flex-col items-center justify-center py-16">
              <svg
                class="w-16 h-16 text-primary-300 dark:text-primary-600 mb-4"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div class="space-y-6">
            ...
            <div class="card-glass hover:shadow-lg transition-shadow">
              ...
              <div class="space-y-4">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div
                    v-if="weekStats.changes.workouts !== null"
                    :class="
                      weekStats.changes.workouts >= 0
                  ...
                  >
                    <Icon
                      v-if="weekStats.changes.workouts >= 0"
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div
                    v-if="weekStats.changes.totalTime !== null"
                    :class="
                      weekStats.changes.totalTime >= 0
                        ? 'text-green-600 dark:text-green-400'
                        : 'text-red-500 dark:text-red-400'
                    "
                    class="flex items-center text-sm font-semibold"
                  >
                    <Icon
                      v-if="weekStats.changes.totalTime >= 0"
                      name="lucide:trending-up"
                      class="w-4 h-4 mr-1"
                    />
                    <Icon v-else name="lucide:trending-down" class="w-4 h-4 mr-1" />
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div
                    v-if="weekStats.changes.avgDuration !== null"
                    :class="
                      weekStats.changes.avgDuration >= 0
                        ? 'text-green-600 dark:text-green-400'
                        : 'text-red-500 dark:text-red-400'
                    "
                    class="flex items-center text-sm font-semibold"
                  >
                    <Icon
                      v-if="weekStats.changes.avgDuration >= 0"
                      name="lucide:trending-up"
                      class="w-4 h-4 mr-1"
                    />
                    <Icon v-else name="lucide:trending-down" class="w-4 h-4 mr-1" />
            ...
            <div class="card-glass hover:shadow-lg transition-shadow">
              ...
              <div v-if="favouriteDay" class="flex items-center gap-4">
                <div
                  class="w-14 h-14 bg-gradient-primary rounded-xl flex items-center justify-center"
                >
                  <span class="text-white font-bold text-lg">{{ favouriteDay.short }}</span>
                </div>
                <div>
                  <p class="text-lg font-bold text-primary-900 dark:text-primary-100 capitalize">
                    {{ favouriteDay.name }}
                  </p>
                  <p class="text-sm text-primary-500 dark:text-primary-400">
                    {{ favouriteDay.count }} entraînement{{ favouriteDay.count > 1 ? 's' : '' }}
                  </p>
                </div>
              </div>
...
const weekStats = computed(() => {
  ...
  const calc = (list: Workout[]) => ({
    ...
    avgDuration:
      list.length > 0
        ? Math.round(list.reduce((s, w) => s + (w.duration || 0), 0) / list.length)
        : 0,
  });
