...
const ACCOUNTS: AccountProfile[] = [
  ...
  // ═══════ FREE ACCOUNTS (5) ═══════
  {
    ...
    rotation: [FULL_BODY, FULL_BODY, FULL_BODY],
    trainingFrequency: 3,
  },
  {
  ...
  },
  {
    ...
    startBodyFat: 20,
    subscription: { plan: SubscriptionPlan.FREE_TRIAL, status: SubscriptionStatus.TRIAL },
    rotation: [FULL_BODY, UPPER, LOWER],
    trainingFrequency: 3,
  },
...
export async function seedTestAccounts() {
  ...
  for (const profile of ACCOUNTS) {
    ...
    if (profile.subscription.status === SubscriptionStatus.ACTIVE) {
      ...
      periodEnd.setMonth(
        periodEnd.getMonth() + (profile.subscription.plan === SubscriptionPlan.YEARLY ? 12 : 1)
      );
    ...
    for (const dayOffset of selectedDays) {
      ...
      for (let eIdx = 0; eIdx < template.exercises.length; eIdx++) {
        ...
        for (let s = 1; s <= tmplEx.sets; s++) {
          const baseWeight =
            tmplEx.weightRange[0] + (tmplEx.weightRange[1] - tmplEx.weightRange[0]) * progress;
          const weight =
            tmplEx.weightRange[1] === 0 ? 0 : Math.round(baseWeight * wf + rand(-2, 2));
    ...
    const templateCount = profile.maxedOut
      ? 2
    ...
    for (let day = profile.historyDays; day >= 0; day -= randInt(2, 5)) {
      ...
      const bf =
        profile.startBodyFat +
        (profile.historyDays - day) * (profile.weightTrend < 0 ? -0.03 : 0.01) +
        rand(-0.3, 0.3);
    ...
    const goalCount = profile.maxedOut
      ? 1
      : profile.subscription.status === SubscriptionStatus.ACTIVE
        ? randInt(3, 4)
        : 1;
    const possibleGoals = [
      ...
      },
      {
        ...
        title: `Atteindre ${Math.round(profile.startWeight + 5)}kg`,
        targetValue: Math.round(profile.startWeight + 5),
        startValue: profile.startWeight,
      },
      {
        type: 'BODY_FAT' as const,
        title: 'Body fat sous 15%',
        targetValue: 15,
        startValue: profile.startBodyFat,
      },
    ];
    ...
    if (profile.historyDays >= 7) {
      const notifs = [
        {
          ...
          title: 'Série de 1 semaine !',
          message: `${profile.firstName}, tu t'entraînes régulièrement depuis 1 semaine.`,
          daysAgo: Math.min(7, profile.historyDays),
        },
      ];
      if (profile.historyDays >= 28) {
        notifs.push({
          ...
          message: 'Tu as battu ton record sur un exercice.',
          daysAgo: randInt(5, 20),
        });
        notifs.push({
          type: NotificationType.STREAK_MILESTONE,
          title: 'Série de 4 semaines !',
          message: '4 semaines consécutives, bravo !',
          daysAgo: 1,
        });
      }
      if (profile.historyDays >= 60) {
        notifs.push({
          ...
          title: 'Progression !',
          message: 'Tu approches de ton objectif.',
          daysAgo: 3,
        });
      }
    ...
    console.log(
      `  ✅ ${profile.email} (${profile.firstName} ${profile.lastName}) — ${workoutCount} workouts, ${subLabel}${profile.maxedOut ? ' [MAXED]' : ''}`
    );
  }
}
