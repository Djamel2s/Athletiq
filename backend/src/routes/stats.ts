...
router.get('/correlation', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    while (iterDate <= now) {
      ...
      weeks.push({
        ...
        avgMaxWeight: data?.maxWeights.length
          ? Math.round((data.maxWeights.reduce((a, b) => a + b, 0) / data.maxWeights.length) * 10) /
            10
          : 0,
      });
    ...
    const lowFreqWeeks = finalWeeks.filter(
      (w) => w.workoutCount > 0 && w.workoutCount < 3 && w.avgMaxWeight > 0
    );

    const highFreqAvgProgress =
      highFreqWeeks.length > 0
        ? Math.round(
            (highFreqWeeks.reduce((sum, w) => sum + w.avgMaxWeight, 0) / highFreqWeeks.length) * 10
          ) / 10
        : 0;

    const lowFreqAvgProgress =
      lowFreqWeeks.length > 0
        ? Math.round(
            (lowFreqWeeks.reduce((sum, w) => sum + w.avgMaxWeight, 0) / lowFreqWeeks.length) * 10
          ) / 10
        : 0;
...
router.post('/check-inactivity', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    await createNotification(userId, NotificationType.INACTIVITY, 'Tu nous manques !', message);
...
router.get('/recovery', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    for (const [muscle, data] of muscleData) {
      const daysSince = Math.floor((now.getTime() - data.lastDate.getTime()) / 86400000);
