...
router.post('/:id/complete', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const result = await withUserLock(userId, 'complete-workout', async () => {
      ...
      checkAndCreatePRNotifications(userId, workout.id).catch((err) =>
        logger.error(
          { err, userId, workoutId: workout.id, route: 'workouts' },
          'PR notification job failed'
        )
      );
      checkStreakMilestone(userId).catch((err) =>
        logger.error({ err, userId, route: 'workouts' }, 'Streak check job failed')
      );
      checkAndUnlockAchievements(userId).catch((err) =>
        logger.error({ err, userId, route: 'workouts' }, 'Achievement check failed')
      );
      ...
      createFeedPostForWorkout(
        ...
        logger.error({ err, userId, route: 'workouts' }, 'Feed post creation failed')
      );
...
router.put('/:workoutId/exercises/:exerciseId', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (data.exerciseLibraryId !== undefined)
      exercise.exerciseLibraryId = data.exerciseLibraryId ?? undefined;
...
// Add set to exercise
router.post(
  '/:workoutId/exercises/:exerciseId/sets',
  authenticate,
...
// Update set
router.put(
  '/:workoutId/exercises/:exerciseId/sets/:setId',
  authenticate,
  async (req: AuthRequest, res) => {
    try {
      const workoutId = parseId(req.params.workoutId);
