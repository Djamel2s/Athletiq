...
export async function generateInsights(userId: number): Promise<CoachInsight[]> {
  ...
  const thisWeekWorkouts = recentWorkouts.filter(
    (w) => w.completedAt && new Date(w.completedAt) >= sevenDaysAgo
  );

  const lastTwoWeeksWorkouts = recentWorkouts.filter(
    (w) => w.completedAt && new Date(w.completedAt) >= fourteenDaysAgo
  );
  ...
  const pushMuscles =
    (muscleCount[MuscleGroup.CHEST] || 0) +
    (muscleCount[MuscleGroup.SHOULDERS] || 0) +
    (muscleCount[MuscleGroup.TRICEPS] || 0);
  ...
  // === 2. Jambes négligées ===
  const legMuscles =
    (muscleCount[MuscleGroup.LEGS] || 0) +
    (muscleCount[MuscleGroup.QUADS] || 0) +
    (muscleCount[MuscleGroup.HAMSTRINGS] || 0) +
    (muscleCount[MuscleGroup.GLUTES] || 0);
  ...
  if (thisWeekWorkouts.length === 0 && recentWorkouts.length > 0) {
    ...
    const daysSince = Math.floor(
      (now.getTime() - new Date(lastWorkout.completedAt!).getTime()) / (1000 * 60 * 60 * 24)
    );
    if (daysSince >= 5) {
      insights.push({
        id: 'inactivity',
  ...
  if (lastTwoWeeksWorkouts.length >= 2) {
    ...
    const lastWeekWorkouts = lastTwoWeeksWorkouts.filter(
      (w) => w.completedAt && new Date(w.completedAt) < sevenDaysAgo
    );
  ...
  if (longWorkouts.length >= 2) {
    insights.push({
      ...
      message:
        'Tes séances dépassent souvent 2h. Des séances plus courtes et intenses sont souvent plus efficaces.',
      priority: 2,
    });
  }
