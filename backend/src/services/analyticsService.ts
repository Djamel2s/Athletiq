...
export async function computeFatigueForUser(userId: number) {
  try {
    ...
    const weeklySql = `SELECT COALESCE(SUM(CAST(s.weight AS FLOAT) * s.reps),0) as load FROM sets s JOIN exercise e ON e.id = s."exerciseId" JOIN workouts w ON w.id = e."workoutId" WHERE w."userId" = $1 AND w.date >= NOW() - INTERVAL '7 days'`
    ...
    const baselineSql = `SELECT COALESCE(SUM(CAST(s.weight AS FLOAT) * s.reps),0) as load28 FROM sets s JOIN exercise e ON e.id = s."exerciseId" JOIN workouts w ON w.id = e."workoutId" WHERE w."userId" = $1 AND w.date >= NOW() - INTERVAL '28 days'`
    ...
    const f = fatigueRepo.create({ userId, date: new Date(), score, source: 'heuristic-v1', notes: `weeklyLoad=${weeklyLoad},baseline=${baselineWeekly}` })
...
export async function getLatestFatigueForUser(userId: number) {
  ...
  const latest = await repo.createQueryBuilder('f').where('f.userId = :userId', { userId }).orderBy('f.date','DESC').getOne()
  return latest
}
