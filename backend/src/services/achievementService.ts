...
async function getUserStats(userId: number): Promise<UserStats> {
  const [workoutCount, totalVolumeResult, bodyEntries, photoCount, user, prCount] = await Promise.all([
    workoutRepo.count({ where: { userId, completedAt: undefined as any } })
      .then(() => workoutRepo.createQueryBuilder('w')
        .where('w.userId = :userId', { userId })
        .andWhere('w.completedAt IS NOT NULL')
        .andWhere('w.isTemplate = false')
        .getCount()
      ),
    workoutRepo.createQueryBuilder('w')
      .select('COALESCE(SUM(w.totalVolume), 0)', 'total')
      .where('w.userId = :userId', { userId })
      .andWhere('w.completedAt IS NOT NULL')
      .getRawOne(),
    bodyStatRepo.count({ where: { userId } }),
    photoRepo.createQueryBuilder('p')
    ...
    `, [userId]).then((r: any[]) => parseInt(r[0]?.count || '0', 10))
  ])
...
function checkCondition(stats: UserStats, condition: { type: string; threshold: number }): boolean {
  switch (condition.type) {
    case 'workout_count': return stats.workoutCount >= condition.threshold
    case 'total_volume': return stats.totalVolume >= condition.threshold
    case 'streak_weeks': return stats.bestStreak >= condition.threshold
    case 'pr_count': return stats.prCount >= condition.threshold
    case 'body_entries': return stats.bodyEntries >= condition.threshold
    case 'photo_count': return stats.photoCount >= condition.threshold
    case 'days_member': return stats.daysMember >= condition.threshold
...
function getProgress(stats: UserStats, condition: { type: string; threshold: number }): { current: number; target: number; percent: number } {
  let current = 0
  switch (condition.type) {
    case 'workout_count': current = stats.workoutCount; break
    case 'total_volume': current = stats.totalVolume; break
    case 'streak_weeks': current = stats.bestStreak; break
    case 'pr_count': current = stats.prCount; break
    case 'body_entries': current = stats.bodyEntries; break
    case 'photo_count': current = stats.photoCount; break
    case 'days_member': current = stats.daysMember; break
  }
  return {
    current,
